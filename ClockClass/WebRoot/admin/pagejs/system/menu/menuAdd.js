  ...
  /*  $("#mname").blur(function(){
    ...
    form.on('submit(menuUser)',function(data){
	 ...
	 /*   //权限名
		...
		$.post("/ControlText/SelectMenuServlet?action=addMenu&mname="+mname+"&url="+mfunction+"&type2="+type2+"&mbtn="+mbtn+"&father="+father,function(data){
			var info = eval("("+data+")");
		//	alert(info);
	 		if(info == 1){
...
//		            parent.location.reload();
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("/ControlText/SelectMenuServlet?action=SelectMenuBtnSon&type="+type,function (res) {
        		var cs = eval('(' + res + ')');
        		var btntype = $('#btnType');
        ...
        $.get("/ControlText/SelectMenuServlet?action=SelectMenuFather&type="+type,function (res) {
        	var cs = eval('(' + res + ')');
